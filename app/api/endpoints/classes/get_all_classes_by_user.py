...
from app.schemas.get_all_classes_by_user import GetAllClassesByUserRequest, GetAllClassesByUserResponse
...
class UseCase:
    ...
    def execute(self, schema: GetAllClassesByUserRequest) -> List[GetAllClassesByUserResponse]:
        classes = self.classes_repo.get_all_classes_by_user(schema.user_id)
        return [
...
class Controller:
    ...
    def handle(self, request: GetAllClassesByUserRequest) -> List[GetAllClassesByUserResponse]:
        try:
...
async def get_all_classes_by_user(
    user_id: str,
    token_user: TokenUser = Security(RequirePermission(PermissionLevelEnum.ADMIN or PermissionLevelEnum.USER or PermissionLevelEnum.PROFESSOR))
):
    use_case = UseCase()
    controller = Controller(use_case=use_case)
    return controller.handle(GetAllClassesByUserRequest(user_id=user_id))